...
const saveToStorageStart = content.indexOf('  private saveToStorage(');
const loadFromStorageStart = content.indexOf('  private loadFromStorage<T>(');

const endOfSave = content.indexOf('  public subscribe(', saveToStorageStart);
...
const before = content.slice(0, loadFromStorageStart);
const after = content.slice(endOfSave);

fs.writeFileSync('src/lib/chatStore.ts', before + newStorage + after);
