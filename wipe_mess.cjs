...
// I know there is only one "class ChatStore {"
// Let's grab everything before it.
const classIdx = content.indexOf('class ChatStore {');
const beforeClass = content.slice(0, classIdx);

// Let's grab the class body
const getProfilesIdx = content.indexOf('  public getProfiles() {');
const endOfFirstClass = content.indexOf('}', getProfilesIdx) + 1;

let classBody = content.slice(classIdx, endOfFirstClass);

// Inside classBody, we want to replace getStoredItem and saveToStorage
// First find getStoredItem
const getStoredIdx = classBody.indexOf('  private getStoredItem<T>');
const subscribeIdx = classBody.indexOf('  public subscribe(', getStoredIdx);
...
classBody = classBody.slice(0, getStoredIdx) + newMethods + classBody.slice(subscribeIdx);

const finalFile = beforeClass + classBody + '\n\nexport const chatStore = new ChatStore();\n';
fs.writeFileSync('src/lib/chatStore.ts', finalFile);
console.log("chatStore.ts rebuilt properly, length:", finalFile.length);
